...
extern __IO FunctionalState	Timer14_Status;
...
/***************************************************************************************************
 *  Function Name: USART2_Initialization
 *
 *  Description:
 *  Input :
 *  Output:
...
void	USART2_IRQHandler( void )
{
	if( USART_GetITStatus( USART2, USART_IT_RXNE) != RESET ){
		...
		if( Timer14_Status == DISABLE){
			TIM14_TimerRunOrStop( ENABLE );
		}
		else{
			TIM14_ClearDelayCounter();
		}
		...
		if( ((digital0 == 'i') && (digital1 == 's')) && ((digital2 == 'O') && (digital3 == 'v')) && ((digital4 == 'E') && (digital5 == 'r')) && ((digital6 == '*') && (digital7 == 0x0D)) ) {
			*isRX_BOOL = true;
			...
			if( USART2_count >= USART2_RX_BufLength ){
				USART2_EnvironmentVariableSet();
				TIM14_TimerRunOrStop( DISABLE );
...
/***************************************************************************************************
 *  Function Name: USART2_VariableSet
 *
 *  Description:
 *  Input :
 *  Output:
...
void	USART2_VariableSwap( void )
{
	if( (isRX_Buf1Full == true) && (isRX_Buf2Full == false) ){
		...
		if( (isRX_Buf1Full == false) && (isRX_Buf2Full == true) ){
			RX_BufPointer = RX_Buf1;
			isRX_BOOL = &isRX_Buf1Full;
		}
		else{
