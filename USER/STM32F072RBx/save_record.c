...
void SaveRecord_WriteInLoraNodePara(void) {
  ...
  if((SystemOperMode == SystemInNormal) && (EnableMaster == true)) {
    FLASH_ReadWord( LoraNodeParaPage1Addr + 4, &temp, 1 );     //起始位址附近取值,因存值都從起始開始存,有值就表示被使用了
    if(temp != 0xFFFFFFFF) {
...
void SaveRecord_ReadOutLoraNodePara(void) {
  ...
  if((SystemOperMode == SystemInNormal) && (EnableMaster == true)) {
    ...
    if(temp != 0xFFFFFFFF) {
      ...
      FLASH_ReadWord( LoraNodeParaPage2Addr + 4, &temp, 1 );     //起始位址附近取值,因存值都從起始開始存,有值就表示被使用了
      if(temp != 0xFFFFFFFF) {
