...
void GPS_ReadIn(tDeviceNodeSensor *Sensor) {
  ...
  if((isGPS_USART_RX_Buf1Full == false) && (isGPS_USART_RX_Buf2Full == false)) {
    return;
  }
  
  if(GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_9) == 0) {   // PB9 = GPS_LDO_EN, PB9 = 0, GPS_LDO off
    goto Return_Out;
  }
  ...
  } else if(isGPS_USART_RX_Buf2Full == true) {
    temp = (uint8_t *)GPS_USART_RX_Buf2;
  }
  ...
  //CmdUART_UartWrite((uint8_t *)"GPS_ReadIn\r\n", strlen("GPS_ReadIn\r\n"));     // for test
