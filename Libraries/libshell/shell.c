...
int	shell_str_cmp( const char *str1, const char *str2, int len1, int len2 )
{
	...
	// make sure we matched a whole command, and not only a containing substring ...
	if( len2 > len1 && str2[i] != ' ' ) return 2;
...
int	shell_process_cmds( const shell_cmd *cmd, unsigned char cmds, const char *cmd_line )
{
	...
	for( i = 0; i < cmds; i++ ) {
		...
		cmd_len = shell_str_len( cmd[i].cmd );

		if( shell_str_cmp( cmd[i].cmd, cmd_line, cmd_len, cmd_line_len ) == 0 ) {
